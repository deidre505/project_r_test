-- main.lua
-- ✅ luasteam 로딩 경로 부트스트랩 (반드시 require들보다 위에!)
do
  local base = ""
  ...
  -- C 모듈(.dll) 검색 경로에 "실행 파일/소스 베이스 디렉토리"를 추가
  -- Windows에서는 이게 luasteam.dll require 성공/실패를 가르는 핵심인 경우가 많음.
  if base ~= "" then
    package.cpath = base .. "\\?.dll;" .. package.cpath
    package.path  = base .. "\\?.lua;" .. base .. "\\?\\init.lua;" .. package.path
  end

  -- 디버그 출력(로그에 남겨서 경로 확인)
  print("[Bootstrap] source base:", base)
  print("[Bootstrap] package.cpath:", package.cpath)
end
